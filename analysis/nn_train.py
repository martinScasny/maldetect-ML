...
x1_train, x1_test, x2_train, x2_test, x4_train, x4_test, y_train, y_test = train_test_split(
    x1,x2,x4,y,test_size=0.2,random_state=42, shuffle=True)

# x2_train, x2_test, y_train, y_test = train_test_split(
#     x2,y,test_size=0.2,random_state=42, shuffle=True)
def createModelNI(learning_rate, in_ngram, in_imports):
  inputNgram = tf.keras.layers.Input(shape=(in_ngram.shape[1],))
  x = tf.keras.layers.Dropout(.3)(inputNgram)
  x = tf.keras.layers.Dense(128, activation='relu')(x)
  x = tf.keras.layers.Dropout(.3)(x)
  x = tf.keras.layers.Dense(128, activation='relu')(x)
  inputImports = tf.keras.layers.Input(shape=(in_imports.shape[1],))
  x1 = tf.keras.layers.concatenate([x, inputImports])
  ...
  model = tf.keras.Model(inputs=[inputNgram, inputImports], outputs=output)
  model.compile(optimizer=tf.keras.optimizers.Adam(lr=learning_rate),
                loss='binary_crossentropy',
                metrics=['accuracy'])
  return model

def createModelBI(learning_rate, in_calls,in_imports):
...
model = createModelBI(learning_rate, x2, x4)
